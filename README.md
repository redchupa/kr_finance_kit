...
[![License: MIT](https://img.shields.io/badge/License-MIT-green?style=flat-square&cacheSeconds=86400)](LICENSE)
...
3. 한 페이지에 다음 입력 (모두 선택사항, 비워둬도 OK):
   ...
   | **상세 attribute 포함** | 52주 고/저, 50일·200일 평균, 일중 고/저, 거래량, 배당, PE 등 추가 attribute (yfinance .info 호출 — 종목 많으면 트래픽 ↑) | ☐ |
...
> 📥 **Import 직후엔 warm-up 필요** — 블루프린트는 등록 즉시 보이지만 첫 알림은 윈도우만큼 기다려야 옵니다 (가격 history를 그만큼 쌓아야 비교 가능).

⚠️ **HA 재시작 시 주의** — coordinator 메모리만 사용하므로 HA를 재시작하면 가격 history가 비워집니다. **60분 윈도우는 재시작 후 약 1시간, 30분은 30분, 15분은 15분 동안 attribute가 None → 자동화 트리거 안 됨.** 운영 중인 HA에서는 평소엔 문제 없고, 본 통합 업데이트(HACS) 후 HA Restart 직후만 warm-up 필요.
...
- `sensor.fi_nikkei` / `_hangseng` / `_ftse` / `_dax` — 글로벌 지수 (`^N225` / `^HSI` / `^FTSE` / `^GDAXI`), 옵션 ON 시
- `sensor.fi_usdkrw` — 환율
- `sensor.fi_kr_<code>` — 한국 종목 가격
  - 기본 attributes: `change`, `change_pct`, `asof`, `prev_close`, `stale`
  - **상세 attribute 옵션 ON 시 추가**: `fifty_two_week_high/low`(+ `_change_pct`), `fifty_day_average` / `two_hundred_day_average`(+ `_change_pct`), `regular_market_day_high/low`, `regular_market_volume`, `market_state`, `currency`, `quote_type`, `long_name`, `short_name`, `average_daily_volume_10_day`, `average_volume`. equity인 경우: `dividend_rate`, `dividend_yield`, `forward_pe`, `trailing_pe`, `pre_market_price`, `post_market_price` (있을 때만)
...
- `binary_sensor.fi_portfolio_pl_alert` — 포트폴리오 평가손익이 ±임계값% 넘으면 ON (옵션에서 활성화)

**HA event bus** (옵션 토글 없음, 항상 fire):
- `kr_finance_kit_kr_market_closed` — 한국 장 마감 시점 (open→closed 전환)
- `kr_finance_kit_us_market_closed` — 미국 장 마감 시점
