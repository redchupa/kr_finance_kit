...
> **Home Assistant 대시보드에 한국 주식이 뜬다.**
> 음성으로 "삼성전자 지금 얼마?"
> 보유 종목 떨어지면 자동 알림.
...
- 🗣️ **자녀나 가족도** "지니야, 삼성전자 얼마야?" 음성으로 물어볼 수 있게
...
- ✨ **종목명 자동 표시** — `sensor.kr_005930`이 자동으로 "삼성전자"로 표시
...
trigger:
  - platform: numeric_state
    entity_id: sensor.kr_005930_005930   # ← 본인 종목으로 교체
...
- `sensor.kr_finance_kit_kospi` / `_kosdaq` — 지수
- `sensor.kr_finance_kit_usdkrw` — 환율
- `sensor.kr_<code>_<code>` — 한국 종목 가격 (attributes: `price`, `change`, `change_pct`, `asof`, `stale`)
- `sensor.us_<symbol>_<symbol>` — 미국 종목 가격
