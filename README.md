...
| 단기 변동률 윈도우 분 | CSV. `15, 30, 60` 형식. 31분 원하면 31 추가 |
...
- "단기 변동률 윈도우 분" 등록 분마다: `change_pct_<N>min` (예 옵션에 `15, 30, 60` → 3개 attribute)
...
v0.1.52부터 **분 단위 자유 입력**. 예: 31분 원하면 옵션 화면 "단기 변동률 윈도우 분"에 31 추가 → 블루프린트의 number 입력에 31 → `change_pct_31min` attribute로 자동 트리거.
...
| ≤v0.1.51 → v0.1.52+ | 단기 변동률 분 단위 자유 입력 — 옵션 "단기 변동률 윈도우 분"에 원하는 분 추가 |
