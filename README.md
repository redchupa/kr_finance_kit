...
- ✨ **종목명 자동 표시** — `sensor.kr_finance_kit_kr_005930`이 자동으로 "삼성전자"로 표시
...
trigger:
  - platform: numeric_state
    entity_id: sensor.kr_finance_kit_kr_005930   # ← 본인 종목으로 교체
...
- `sensor.kr_finance_kit_kospi` / `_kosdaq` — 한국 지수
- `sensor.kr_finance_kit_nasdaq` / `_dow` / `_sp500` — 미국 지수 (`^IXIC` / `^DJI` / `^GSPC`)
- `sensor.kr_finance_kit_usdkrw` — 환율
- `sensor.kr_finance_kit_kr_<code>` — 한국 종목 가격 (attributes: `price`, `change`, `change_pct`, `asof`, `stale`)
- `sensor.kr_finance_kit_us_<symbol>` — 미국 종목 가격
- `sensor.kr_finance_kit_other_<slug>` — 암호화폐·환율·선물 (예 `_btc_usd`, `_eth_usd`, `_eur_x`, `_gc_f`). 24/7 갱신
- `sensor.kr_finance_kit_portfolio_*` — 6개 평가 sensor (KR/US/KRW 환산 × value/pl)
- `binary_sensor.kr_finance_kit_disclosure_<corp_code>` — 24h 윈도우 공시 트리거
...
## 🔄 v0.1.32 마이그레이션 (한국어 entity_id → 영어)

v0.1.31 이전 버전에서는 device 친절명이 한국어("한국 시장 지표")라서 HA가 entity_id를 한국어 슬러그로 만들었습니다 (`sensor.hangug_sijang_jipyo_kospi`). v0.1.32부터는 `_attr_suggested_object_id` 로 모든 신규 entity가 영어 슬러그(`sensor.kr_finance_kit_kospi` 등)로 생성됩니다. 이미 등록된 한국어 entity는 HA registry에 영구 저장되어 자동으로 안 바뀝니다.

영어 entity_id로 정렬하려면 (한 번만 하면 됨):

1. **간단**: 설정 → 기기 및 서비스 → KR Finance Kit → ⋮ → **삭제** → 다시 추가. 보유 종목(quantity·평단가)은 `kr_finance_kit.add_position` 서비스로 다시 입력.
2. **수동**: 설정 → 기기 및 서비스 → entities → 각 entity 클릭 → 톱니바퀴 → entity ID 수정. 자동화에서 참조한 entity_id도 함께 업데이트 필요.

그 후 본문의 블루프린트 import (또는 다시 import) 하면 종목 드롭다운에 영어 entity_id가 깔끔하게 표시됩니다.
